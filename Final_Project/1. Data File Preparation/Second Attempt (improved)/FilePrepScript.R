...
# Remove sparse terms
tdm1 <- removeSparseTerms(tdm1, 0.01)
...
# Remove sparse terms
tdm2 <- removeSparseTerms(tdm2, 0.01)
...
# Remove Sparse ters
tdm3 <- removeSparseTerms(tdm3, 0.01)
...
# Remove Sparse ters
tdm4 <- removeSparseTerms(tdm4, 0.01)
...
# write data to a file
write.csv(unigramDf, "./data_frames/unigrams.csv", row.names=FALSE)
...
unigrams <- read.csv("./data_frames/unigrams.csv", stringsAsFactors=FALSE)
...
# Update unigram data frame
w1 <- word(unigrams$item, 1)
unigrams <- data.frame(w1=w1, rank=unigrams$Frequency)
...
names(bigrams)[names(bigrams)=="prediction"] <- "w2"
...
names(trigrams)[names(trigrams)=="prediction"] <- "w3"
...
names(quadgrams)[names(quadgrams)=="prediction"] <- "w4"


############### Convert factors to strings #################################

unigrams$w1 <- as.character(unigrams$w1)

bigrams$w1 <- as.character(bigrams$w1)
bigrams$w2 <- as.character(bigrams$w2)

trigrams$w1 <- as.character(trigrams$w1)
trigrams$w2 <- as.character(trigrams$w2)
trigrams$w3 <- as.character(trigrams$w3)

quadgrams$w1 <- as.character(quadgrams$w1)
quadgrams$w2 <- as.character(quadgrams$w2)
quadgrams$w3 <- as.character(quadgrams$w3)
quadgrams$w4 <- as.character(quadgrams$w4)
...
# write data to a file
write.csv(unigrams, "./data_frames_ready/unigrams.csv", row.names=FALSE)
...
rm(bigramDF, trigramDF, conn, corpus, files)
...
############# processing probabilities and ranks ##################################################
